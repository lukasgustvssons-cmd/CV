...
export function PricingCard({ name, price, description, features, highlighted = false }: PricingCardProps) {
  return (
    ...
    >
      ...
      <p className={`mt-2 text-sm ${highlighted ? 'text-slate-300' : 'text-slate-600'}`}>{description}</p>
      <p className="mt-6 text-4xl font-semibold tracking-tight">{price}</p>
      <ul className="mt-6 space-y-3 text-sm">
        {features.map((feature) => (
          <li key={feature} className={highlighted ? 'text-slate-200' : 'text-slate-700'}>
            {feature}
          </li>
        ))}
      </ul>
      ...
      >
        {highlighted ? 'Get Pro' : 'Choose Plan'}
      </button>
    </article>
  );
}
